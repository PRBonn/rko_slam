...
TEST_CASE("voxel_iou: identical clouds -> IoU = 1", "[voxel_iou]") {
  const std::vector<Eigen::Vector3f> first_cloud = grid_block(8, 8, 8, Eigen::Vector3f::Zero(), 0.5F);
  const auto iou = rko_slam::core::voxel_set_iou(first_cloud, first_cloud, Sophus::SE3f{}, 0.5F);
  REQUIRE(iou == 1.0);
}

TEST_CASE("voxel_iou: disjoint clouds -> IoU = 0", "[voxel_iou]") {
  ...
  const auto iou = rko_slam::core::voxel_set_iou(first_cloud, second_cloud, Sophus::SE3f{}, 0.5F);
  REQUIRE(iou == 0.0);
}

TEST_CASE("voxel_iou: raw inputs at finer-than-IoU spacing - IoU stays <= 1", "[voxel_iou]") {
  // A `|second| := second_pts.size()` denominator would give IoU = 1.5 here.
  ...
  const auto iou = rko_slam::core::voxel_set_iou(first_cloud, second_cloud, Sophus::SE3f{}, 1.0F);
  REQUIRE(iou == 0.5);
}

TEST_CASE("voxel_iou: half-overlapping clouds -> IoU in [0.5, 0.55]", "[voxel_iou]") {
...
TEST_CASE("voxel_iou: first_T_second transforms second_pts into first's frame", "[voxel_iou]") {
  ...
  REQUIRE(rko_slam::core::voxel_set_iou(first_cloud, second_cloud, first_T_second, 1.0F) == 1.0);
}

TEST_CASE("voxel_iou: asymmetric sizes - denom is min(|F|, |S|)", "[voxel_iou]") {
  const std::vector<Eigen::Vector3f> first_cloud = grid_block(4, 1, 1, Eigen::Vector3f(0.5F, 0.5F, 0.5F), 1.0F);
