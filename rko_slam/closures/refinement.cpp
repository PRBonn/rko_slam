...
ClosureRefinement refine_closure(const float voxel_size,
  ...
  const Sophus::SE3f refined = icp_point_to_plane(source_centroids, target_centroids, target_normals,
                                                  max_correspondence_distance, target_T_source_init);

  const double overlap = voxel_overlap_coefficient(target_centroids, source_centroids, refined, voxel_size);
